...
class AuthMiddleware implements MiddlewareInterface
{
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        if ($container) {
            ...
            $stmt = $pdo->prepare("SELECT lastModifiedPassword, active FROM users WHERE id = :id");
            $stmt->execute(['id' => $_SESSION['user']['id']]);
            $userDb = $stmt->fetch(\PDO::FETCH_ASSOC);

            if (!$userDb || (int)($userDb['active'] ?? 1) === 0) {
                // Le compte a été désactivé (ou supprimé) ! Invalider la session
                unset($_SESSION['user']);
            ...
            if ($userDb['lastModifiedPassword'] !== ($_SESSION['user']['last_password_modified'] ?? null)) {
